		    ...
		    <div class="content-grids">
		    	...
		    	<c:if test = "${list.size() != 0}">
            		...
            		<c:if test = "${currentPage != 1}">
                		<a href="/laptopshop/store?page=${currentPage-1}&range=${range}&brand=${brand}&manufactor=${manufactor}&os=${os}&ram=${ram}&cpu=${cpu}">Back</a>
                	</c:if>
                	<c:if test = "${currentPage == 1}">	
        				<a class="current">1</a>
        			</c:if>
        			<c:if test = "${currentPage != 1}">	
        				<a href="/laptopshop/store?page=1&range=${range}&brand=${brand}&manufactor=${manufactor}&os=${os}&ram=${ram}&cpu=${cpu}">1</a>
        			</c:if>
        			...
        			<c:forEach var="pag" items="${pageList}" varStatus="loop">
        				...
        				<c:if test = "${currentPage != pag}">	
        					<a href="/laptopshop/store?page=${pag}&range=${range}&brand=${brand}&manufactor=${manufactor}&os=${os}&ram=${ram}&cpu=${cpu}">${pag}</a>
        				</c:if>
      				</c:forEach>

            		<c:if test = "${currentPage != totalPage}">
                		<a href="/laptopshop/store?page=${currentPage+1}&range=${range}&brand=${brand}&manufactor=${manufactor}&os=${os}&ram=${ram}&cpu=${cpu}">Next</a>	
                	</c:if>
				</div>
		    	</c:if>
		    	...
		    	<div class="content-sidebar">
		    		<h4 style="color: dodgerblue;"><b>Lọc sản phẩm</b></h4>		    			
        				...
        				<c:if test = "${brand == 'LAPTOP' || brand == 'LAPTOP GAMING' ||brand == 'LAPTOP VĂN PHÒNG'}">
        				<p>Hệ điều hành</p>
        				<div class="select-range">
						...
						<div class="select-range">
        					<select name="cpu" class="form-control">
								...
								<option value="Core">Intel Core i</option>
